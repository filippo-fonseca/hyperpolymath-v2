// Mobile port of apps/web/components/jarvis/jarvis-input-payload.ts.
// Same parse chain (slash → $projects → #hashtags → dates → priority) minus
// the TipTap editor JSON — mobile is plain text, so $project mentions are
// resolved by matching tokens against the cached project list.
...
import type { ProjectRef } from "./jarvis-context";
