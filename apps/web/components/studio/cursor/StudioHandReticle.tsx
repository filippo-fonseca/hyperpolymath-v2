...
/**
 * StudioHandReticle — the on-brand brass hand cursor for the Studio.
 *
 * In hand mode there is no OS cursor, so this is the ONLY thing telling the user
 * where they're aiming. It renders a candlelit brass ring + parchment dot that
 * follows the shared input cursor at 60fps, snaps larger when it hovers a widget,
 * and fires one-shot pulses on expand / collapse / swipe intents.
 *
 * It is a SEPARATE production component from `StudioDebugCursor` (which stays
 * behind `?studioDebug=1` and owns the Playwright `data-studio-*` contract). Its
 * observability attributes use a disjoint `data-reticle-*` namespace.
 *
 ...
 * In mouse mode `cursor.active` is true the moment the pointer enters the stage,
 * but the OS cursor already exists there, so a second ring would be redundant
 * chrome. The component always mounts (hooks stay unconditional) and gates via
 * opacity, exactly like the debug cursor.
 *
 * KNOWN QUIRK (accepted): hand control is additive — the mouse driver keeps
 * running — so while hand mode is on, a mouse move also writes the shared hub
 * cursor and the ring tracks whichever source moved last. That's still "where
 * Studio thinks you're aiming," so it's harmless.
 *
 ...
 *
 * Palette is inlined (not imported from `materials/tokens.ts`) because that
 * module imports `three` at module scope; a DOM overlay must not drag three into
 * the eagerly-loaded chunk. Matches the onboarding/overlay convention.
 */
...
import type { StudioIntentType } from "@/lib/studio/input/types";
import { useActiveWidget } from "@/lib/studio/state/active-widget";
import { useHandStatus } from "@/lib/studio/state/hand-status";
...
export function StudioHandReticle(): React.JSX.Element {
  ...
  const focusedWidget = useActiveWidget();
  ...
  // child so its CSS animation restarts), cleared after FEEDBACK_MS. Mirrors the
  // debug cursor's lastIntent/timerRef pattern, including cleanup on replace.
  ...
  const dimmed = focusedWidget !== null;
  ...
  return (
    <div
      ...
      data-reticle-dimmed={dimmed ? "true" : "false"}
    ...
    >
      ...
      {!reduced && feedback ? (
        <Pulse key={feedback.key} type={feedback.type} />
      ) : null}
    </div>
  );
}
...
/** Component-scoped keyframes — no global CSS / Tailwind @theme edits. */
