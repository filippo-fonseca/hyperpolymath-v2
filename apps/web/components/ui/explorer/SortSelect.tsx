    ...
    >
      <SelectPrimitive.Trigger
        ...
        className={cn(
          ...
          "hover:bg-[var(--sd-hover)] focus-visible:border-[var(--sd-accent)]",
          className
        )}
      >
          ...
          <SelectPrimitive.Viewport>
            {(Object.keys(sortLabels) as ExplorerSortValue[]).map((option) => (
              <SelectPrimitive.Item
                key={option}
                value={option}
                className="relative flex h-7 select-none items-center rounded-[4px] px-2 pr-7 outline-none transition-colors duration-[120ms] ease-out focus:bg-[var(--sd-menu-hover)] data-[state=checked]:text-[var(--sd-accent)]"
              >
