...
/**
 * ModeToggle.tsx — U-15 · The Studiolo · mode-toggle
 *
 * The Cmd+\ escape hatch that flips between the 2D "Page" (the existing LifeOS
 * UI) and the 3D "/world", with a brief fade through Nightwalnut.
 *
 ...
 *
 * Cooperation with the two existing keyboard systems:
 ...
 *   - `camera/useWorldKeys.ts` (capture-phase window listener, only mounted on
 *     /world) bails immediately on `e.metaKey || e.ctrlKey`, so Cmd+\ passes
 *     straight through it. That means this single listener handles Cmd+\ from
 *     BOTH the 2D Page and the 3D world.
 *
...
/** Remembers the 2D route to return to, so World → Page lands where you left. */
...
export function ModeToggle() {
  ...
  const navigate = useCallback(() => {
    ...
    if (inWorld) {
      ...
      dest = stored && !stored.startsWith(WORLD_PATH) ? stored : DEFAULT_PAGE_PATH;
    } else {
      try {
    ...
    setCovering(true);
  }, [inWorld, pathname, reduceMotion, router]);
  ...
  return (
    <>
      <button
        ...
        aria-label={
          inWorld
            ...
            : "Enter the Studiolo (Command Backslash)"
        }
        title={inWorld ? "Return to the Page" : "Enter the Studiolo"}
        className={cn(
      ...
      >
        <span>{inWorld ? "Return to the Page" : "Enter the Studiolo"}</span>
        <kbd
