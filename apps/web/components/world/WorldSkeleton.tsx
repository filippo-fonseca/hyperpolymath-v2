...
/**
 * WorldSkeleton — the loading state shown while the ssr:false Canvas island
 * (WorldCanvas + three/R3F) streams in (U-02).
 *
 * A paper-textured Nightwalnut void with a single CSS-pulsing candle-point at
 * center — the world holding its breath before the Litany. No three import;
 * pure DOM + inline styles on the STUDIOLO brand tokens so it renders instantly
 * before any 3D bytes arrive.
 */
export function WorldSkeleton(): React.ReactElement {
  return (
    <div
      aria-hidden="true"
      style={{
    ...
    >
      <span
        style={{
          ...
          animation: "studiolo-candle 1.8s ease-in-out infinite",
        }}
      />
      <style>{`
        @keyframes studiolo-candle {
