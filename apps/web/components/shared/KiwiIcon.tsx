/**
 * KiwiIcon — Lucide-compatible kiwi-bird glyph.
 *
 * Source: apps/web/public/icons/kiwi-bird.svg (24×24 viewBox, single
 * filled path). Mirrors Lucide's React API (size, strokeWidth, className,
 * color via currentColor) so it can drop into the sidebar's `icon:` slot
 * alongside other Lucide icons.
 *
 * Used in:
 *   - components/shell/PersistentNav.tsx — JARVIS nav item
 *   - components/landing/MeetKiwiSection.tsx — landing explainer
 *   - any other surface that wants the brand-aligned agent glyph
 */

interface KiwiIconProps {
  size?: number | string;
  strokeWidth?: number; // accepted for API compatibility; the source path is fill-based
  className?: string;
  color?: string;
