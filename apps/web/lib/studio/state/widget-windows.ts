...
import { clampToStage, nextStackOrder, pickSpawnPosition } from "../windows/layout";
...
export function useWidgetWindows(): readonly WidgetWindowInstance[] {
  return useSyncExternalStore(subscribeWidgetWindows, getWidgetWindows, () => EMPTY);
}
    ...
    windows = parsed
      .filter((item): item is WidgetWindowInstance => {
        ...
        return typeof value.id === "string" && typeof value.kind === "string" &&
          [value.x, value.y, value.w, value.h, value.z, value.createdAt].every(Number.isFinite);
      })
      .map((item) => ({ ...item, ...clampToStage(item), props: item.props ?? {} }));
    emit();
  } catch {
    windows = EMPTY;
  }
...
export function summonWidget(
  ...
  options?: { defaultSize?: { w: number; h: number }; singleton?: boolean },
): string {
  const singleton = options?.singleton;
  const existing = singleton ? windows.find((item) => item.kind === kind) : undefined;
  if (existing) {
  ...
  write([
    ...windows,
    { id, kind, props, ...rect, z: nextStackOrder(windows), createdAt: Date.now() },
  ]);
  return id;
}

export function moveWidget(id: string, x: number, y: number): void {
  write(windows.map((item) => item.id === id ? { ...item, ...clampToStage({ ...item, x, y }) } : item));
}

export function resizeWidget(id: string, w: number, h: number): void {
  write(windows.map((item) => item.id === id ? { ...item, ...clampToStage({ ...item, w, h }) } : item));
}

export function updateWidgetProps(id: string, props: Record<string, unknown>): void {
  write(windows.map((item) => item.id === id ? { ...item, props: { ...item.props, ...props } } : item));
}
...
export function focusWidget(id: string): void {
  ...
  write(windows.map((candidate) => candidate.id === id ? { ...candidate, z: nextStackOrder(windows) } : candidate));
}
