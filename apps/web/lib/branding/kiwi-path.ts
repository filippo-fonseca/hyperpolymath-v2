/**
 * The kiwi-bird silhouette, as a bare SVG path on a 24x24 viewBox.
 *
 * This lives in its own leaf module rather than beside the other brand geometry
 * in `svg.tsx` because that file imports `next/og` (via `@/lib/og/fonts`) at
 * module scope. The in-app lockup is a client component, and importing the
 * constant from `svg.tsx` would drag the whole OG image pipeline into the
 * browser bundle. Everything that draws the mark — the downloadable SVG, the
 * PNG renderer, and the sidebar — reads it from here, so the geometry has one
