...
//
// Authed ingest endpoint for the local iMessage sync worker (separate unit
// `imessage-sync`). The worker reads new rows out of the macOS Messages
// chat.db (~/Library/Messages/chat.db) and POSTs batches here; we upsert
// them into imessage_messages, keyed on (user_id, chat_jid, external_id)
// so replays are safe.
//
// Auth mirrors /api/whatsapp/ingest exactly — same `Authorization: Bearer
// hpd_...` per-device token, validated via validateDesktopBearerIdentity.
// This gives multi-device readiness while keeping the single-user personal
// deployment unchanged.
//
// The read_imessage tool (server-side, runs inside JARVIS turns) queries
// the same table, so once messages land here they are immediately available
// to the agent + daily briefings with zero mid-turn desktop round-trip.
...
const IngestMessageSchema = z
  .object({
    ...
    /** ISO 8601 timestamp; the worker converts chat.db's Apple-epoch column. */
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const result = await db
      ...
      .onConflictDoNothing({
        target: [
          imessageMessages.userId,
          imessageMessages.chatJid,
          imessageMessages.externalId,
        ],
      })
    ...
    console.error("[imessage/ingest] insert failed", err);
    return Response.json(
