/**
 * React bindings for the Studio input core (desktop port).
 *
 ...
 *
 * Desktop divergence from the web source: the desktop has a real OS mouse, so no
 * MouseKeyboardDriver is registered by default — the provider mounts with zero
 * drivers and the hand driver is registered on demand by the tracking layer when
 * the toggle turns on.
 */
...
export type StudioInputProviderProps = {
  ...
  /** Drivers to register on mount. Defaults to none (the desktop mouse is real). */
...
/** The hover seam — register a custom hover provider (e.g. an elementFromPoint hit-test). */
