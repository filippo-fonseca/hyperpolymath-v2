...
export const findTasksTool = {
  ...
  description:
    "Fuzzy-find tasks by title text and/or filters. Omit any filter you are not using. Returns up to 10 matches with their ids. Use BEFORE update_task or delete_task when the target task isn't already in session entities. `query` is a free-text search against task titles. `status` filters to one or more status values. `priority` filters to one or more priority values.",
