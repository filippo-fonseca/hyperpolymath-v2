...
### ✅✅ PHASE 3 "THE BOTTEGA" — CODE-COMPLETE & VERIFIED (2026-07-07)
Final integration gate on `lifeos-studiolo`: non-baseline tsc = 0; world Vitest 202/202 (11 files); `pnpm --filter web build` exit 0. All 18 planned units done (16 build + 2 stretch W-17/W-18 deferred pending Filippo). The Ring is gone; the five-widget bench (Tasks/Captures/Agenda/Habits/Journal) stands on the arc around the untouched Tree; swipe (wheel + ←/→ + C), grab-and-move (slot reorder, localStorage-persisted), the focused-panel heroGlass backplate, and the "The bench is yours." greeting all shipped. W-14 honesty-sweep found zero gaps. W-15 proved the static draw-call budget (≤75 vs ≤90 ceiling; idle→0 rAF). W-16 rewrote README (meridian→bench + add-a-widget recipe) + CHANGELOG.

### ⏳ HUMAN GATE 2 — Filippo's authenticated in-browser smoke test (PHASE-3-PLAN §8)
Everything CI-checkable is green; the live runtime rows need Filippo's Supabase session on `/world` (+ `?perf=1`). Checklist (perf.md §B3 + §8 in-browser): boot→Litany→5 panels on the arc w/ Tree down the aisle + live data per panel; two-finger swipe + ←/→ glide; C summons Agenda; click placard summons; Esc walks home; 1–9 still fly to boughs; complete a task → ember ascent + bell; resolve/create capture cross-theatre; toggle habit cross-theatre; Agenda matches /calendar, class event wears bough hue, Jarvis-created event shimmers in, disconnect gcal → panel darkens; Journal shows entry + Open-on-Page; grab/drag/drop a panel → preview-shift + dock chime + persists on reload + Esc-cancel; focused panel wears glass backplate; macOS Reduce Motion → instant cuts; Cmd+\ round-trip byte-identical. **Do NOT push/PR until Filippo blesses the smoke test.** Branch is `lifeos-studiolo`, not pushed.
