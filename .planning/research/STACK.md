...
**Confidence:** HIGH (versions and patterns verified against official 2026 docs)

---

## Executive Summary

The user's high-level stack — Next.js + TypeScript strict + Tailwind + Supabase + Anthropic Claude Sonnet 4.6 + Vercel + Vitest — is the modern 2026 default for this exact class of app (single-user, realtime, AI-driven, opinionated UI). This document locks in the *specific* versions, glue libraries, and patterns that connect them well, and surfaces what NOT to use (especially v1 carryovers that no longer fit).
...
```bash
# Bootstrap
npx create-next-app@latest hyperpolymath-v2 --typescript --tailwind --app --turbopack --use-npm
...
# Date utilities
npm install date-fns chrono-node
...
**`proxy.ts`** (replaces `middleware.ts` in Next 16) — refreshes auth tokens on every request:
```ts
import { createServerClient } from '@supabase/ssr';
...
export async function proxy(request: NextRequest) {
  ...
  const supabase = createServerClient(/* ...same config, with cookies wired to request/response... */);
  await supabase.auth.getClaims(); // CRITICAL: getClaims, NOT getSession
  return response;
}
...
**Security rule (HIGH confidence, from Supabase 2026 docs):**
...
import { authUsers } from 'drizzle-orm/supabase'; // built-in Supabase helpers
...
const client = postgres(process.env.DATABASE_URL!, { prepare: false }); // prepare:false for Supabase pooler
export const db = drizzle(client, { schema });
```

**RLS is enforced at the Postgres layer regardless of which client you use** — Drizzle queries with the service role key bypass RLS, queries with the anon key don't. Use the pooler URL for Drizzle (`postgresql://...pooler.supabase.com:6543/postgres`) so you respect RLS based on the connection role.

For RLS policy definitions, use Drizzle's built-in helpers:
```ts
...
**The 2026 pattern (HIGH confidence):**
...
**RLS + Realtime gotcha:** Realtime Postgres Changes respect RLS only if you configure them to. Add an RLS policy to `realtime.messages` *and* enable RLS on the source table. See "Pitfalls" doc for details.

### 4. Anthropic Claude — Streaming + Strict Tool Use for Kiwi

**Model:** `claude-sonnet-4-6` (verified in Anthropic platform docs as of May 2026).

**Pattern: Strict tool use is the right primitive for Kiwi's multi-action JSON contract** — better than free-form JSON output mode because:
...
export async function POST(req: Request) {
  ...
  const stream = client.messages.stream({
    ...
    max_tokens: 4096,
    system: [
      { type: 'text', text: KIWI_PROMPT, cache_control: { type: 'ephemeral' } },
      { type: 'text', text: `Today: ${context.today}\nProjects: ${JSON.stringify(context.projects)}` },
    ],
    tools: [
      {
        name: 'create_task',
        description: 'Create a new task in Hyperpolymath',
        input_schema: z.toJSONSchema(createTaskSchema), // Zod 4 native
      },
      // create_capture, create_event...
    ],
...
**Prompt caching:** Mark the system prompt and tool definitions with `cache_control: { type: 'ephemeral' }`. Default TTL is 5 minutes (changed from 1hr in March 2026). For Kiwi's ~10K-token system prompt, expect ~90% cost reduction on follow-up turns within the same session.

**Streaming:** Use `client.messages.stream()` (returns event emitter + SSE-ready stream) over `client.messages.create({ stream: true })` (raw async iterator). The former integrates cleanly with Next.js Response streaming.

**Thinking-word indicator (v1 carryover):** Render the thinking word locally on the client while waiting for the first stream event. Don't try to stream it from the server.

### 5. Fonts (EB Garamond + Louize)

**EB Garamond** is on Google Fonts — load via `next/font/google`:
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return <html lang="en" className={ebGaramond.variable}>{children}</html>;
}
```

**Louize** is a commercial font (Matthieu Cortat, ~€250 for desktop+web). Once licensed, drop `.woff2` files in `app/fonts/` and use `next/font/local`:
...
const louize = localFont({
  src: [
    { path: '../fonts/Louize-Regular.woff2', weight: '400', style: 'normal' },
    { path: '../fonts/Louize-Italic.woff2', weight: '400', style: 'italic' },
    { path: '../fonts/Louize-Bold.woff2', weight: '700', style: 'normal' },
  ],
...
Then in `app/globals.css` (Tailwind 4 CSS-first config):
```css
@theme {
  --font-serif: var(--font-louize, var(--font-eb-garamond), Georgia, serif);
}
```

**Pitfall:** A common mistake with local fonts is mapping the wrong weight to the wrong file (filename says "Bold" but internal metadata is different). Verify by inspecting font internals or rendering each weight side by side.
...
export default defineConfig({
  ...
  test: {
    environment: 'jsdom', // happy-dom is faster for Next 16
...
**Critical path tests for v2 (per PROJECT.md constraints):**
1. Kiwi tool-call output → `executeAction` round-trip (the JSON contract that v1 lost sleep over)
2. Date/time/priority parsers (the parsing that has to "just work")
3. Project/hashtag reference resolution (`$projectname` → ID, `#tag` → tag)

**Defer:** UI tests, full Server Component tests, e2e (Playwright). Per PROJECT.md, "skip UI tests for MVP."
...
**If you decide to add a CLI later (post-MVP, per PROJECT.md "Out of Scope"):**
...
**If single-user assumption breaks (multi-user features):**
...
**If you want server-side LLM rate limiting / observability:**
...
*Stack research for: Hyperpolymath v2 (personal life-OS web app with Kiwi NLP agent)*
*Researched: 2026-05-07*
