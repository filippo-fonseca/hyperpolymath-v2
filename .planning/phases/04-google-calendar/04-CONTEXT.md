...
- **D-05: Refresh tokens encrypted at-rest via app-level AES-256-GCM (`node:crypto`).** *Revised 2026-05-12 after research — supersedes pgcrypto recommendation.* Phase 1 shipped plain-text `users.gcal_*` columns (intentional placeholder). Phase 4 encrypts via `node:crypto` AES-256-GCM in the Server Action layer before insert; stores ciphertext as `bytea`. Encryption key lives in env var `GCAL_TOKEN_ENC_KEY` (32 bytes, base64-encoded) — never touches the DB wire. Supabase Vault was investigated and rejected: Vault requires `service_role`, Server Actions run as `authenticated` → permission denied. App-level encryption is simpler, NIST-standard, and keeps the key boundary clean. **Migration shape (additive):** ADD COLUMN `gcal_refresh_token_encrypted bytea`, `gcal_access_token_encrypted bytea`; app reads encrypted-with-plain-fallback during transition; follow-up migration drops the plain columns. Plain columns MUST be dropped before Phase 4 ships to production (security non-negotiable).
