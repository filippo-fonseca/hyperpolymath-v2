...
- [ ] 20-02-PLAN.md — Server actions app/actions/journal.ts: upsertJournalEntry (onConflictDoUpdate on UNIQUE(user_id,date), field-preserving patch), getJournalEntry, getJournalEntries — getClaims auth, Zod, ActionResult, no revalidatePath + upsert-idempotency test (JOURNAL-SERVICE-01)
